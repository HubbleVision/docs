...
const API_CONFIGS: ApiConfig[] = [
  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        method: 'POST',
        path: '/generate-chart',
        description: '自然语言→SQL→执行→自动选择图表',
        sampleBody: {
...
export default function ApiPlayground(): JSX.Element {
  ...
  const endpoint = useMemo(() => api.endpoints.find((e) => e.id === endpointId)!, [api, endpointId]);
  ...
  const [bodyText, setBodyText] = useState<string>(endpoint.sampleBody ? pretty(endpoint.sampleBody) : '');
  ...
  const sendRequest = useCallback(async () => {
    ...
    try {
      ...
      const isStream = endpoint.supportsStream && (() => {
        try {
      ...
      if (isStream) {
        ...
        setRespHeaders(Array.from(res.headers.entries()).map(([k,v]) => `${k}: ${v}`).join('\n'));
        ...
        if (reader) {
          ...
          setRespBody('SSE 流不可用（Response.body 为空）');
        }
      } else {
        const res = await fetch(url, init);
        const time = Math.round(performance.now() - start);
    ...
    <div className={styles.container}>
      <div className={styles.sidebar}>
        <div className={styles.block}>
          <div className={styles.blockTitle}>选择 API</div>
          <select value={apiId} onChange={(e) => setApiId(e.target.value as any)} className={styles.select}>
            {API_CONFIGS.map((a) => (
        ...
        <div className={styles.block}>
          <div className={styles.blockTitle}>端点</div>
          ...
          {endpoint.description && <div className={styles.muted}>{endpoint.description}</div>}
        </div>
        <div className={styles.block}>
          <div className={styles.blockTitle}>认证</div>
          <div className={styles.kvRow}>
            <div className={styles.kvKey}>{api.apiKeyHeader}</div>
            <input className={styles.input} value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="your-api-key" />
          </div>
        </div>
        <div className={styles.block}>
          <div className={styles.blockTitle}>请求头（JSON）</div>
          <textarea className={styles.textarea} value={headersText} onChange={(e) => setHeadersText(e.target.value)} />
        </div>
      </div>

      <div className={styles.main}>
        <div className={styles.row}>
          <select value={method} onChange={(e) => setMethod(e.target.value as HttpMethod)} className={styles.methodSelect}>
            {['GET','POST','PUT','PATCH','DELETE'].map(m => <option key={m} value={m}>{m}</option>)}
          </select>
          <input className={styles.urlInput} value={url} onChange={(e) => setUrl(e.target.value)} />
          <button className={styles.sendBtn} disabled={loading} onClick={sendRequest}>{loading ? '发送中…' : '发送'}</button>
        </div>

        {method !== 'GET' && (
          <div className={styles.block}>
            <div className={styles.blockTitle}>请求体（JSON）</div>
            <textarea className={styles.codeArea} value={bodyText} onChange={(e) => setBodyText(e.target.value)} />
            {endpoint.supportsStream && <div className={styles.muted}>提示：Text2SQL/Chart 设置 "stream": true 时将以 SSE 流返回。</div>}
          </div>
        )}

        <div className={styles.block}>
          <div className={styles.blockTitle}>响应</div>
